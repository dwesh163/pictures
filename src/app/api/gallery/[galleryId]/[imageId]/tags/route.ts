...
export async function POST(req: NextRequest, { params }: PageProps) {
	...
	try {
		...
		const { tags } = requestBody as { tags: Tag[] };
		...
		if (newTags.length > 0) {
			...
			const deleteParams = [params.imageId, ...newTags.map((tag) => tag.id)];

			await connection.query(deleteQuery, deleteParams);

			const currentTags = await connection.query('SELECT tagId FROM image_tags WHERE imageId = ?', [params.imageId]);

			for (const tag of newTags) {
				const tagExists = (currentTags[0] as RowDataPacket[]).find((currentTag: RowDataPacket) => currentTag.tagId === tag.id);
				if (!tagExists) {
					await connection.query('INSERT IGNORE INTO image_tags (imageId, tagId) VALUES (?, ?)', [params.imageId, tag.id]);
				}
			}
		} else {
